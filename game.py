...
class Game:
    ...
    def process_event(self, event):
        ...
        if event.type == KEYDOWN:
            ...
            # Manual switch to proceed the solution
            if event.key == K_m and self.solution_ready == True:
                self.execute_moves = True
        if event.type == MOUSEBUTTONUP:
            self.player_interface.click(event.pos, self.level, self)
        if event.type == MOUSEMOTION:
            self.player_interface.mouse_pos = event.pos
        if self.execute_moves:
            move("Thread-1", 2, self.strategy)
            self.execute_moves = False
            self.solution_ready = False
    ...
    def solve_and_move(self):
        ...
        if strategy is not None:
            ...
            print("Solution Ready. Press M to proceed")
    ...
    def auto_move(self):
        ...
        try:
            ...
            thread = threading.Thread(target = self.solve_and_move)
            thread.daemon = True
            thread.start()
        except:
