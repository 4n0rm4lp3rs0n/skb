    ...
    while frontier:
        ...
        if node[-1] not in exploredSet:
            ...
            for action in legalActions(node[-1][0], node[-1][1]):
                ...
                frontier.push(node + [new_state], cost(node_action) + 1)
                actions.push(node_action + [action[-1]], cost(node_action) + 1)
        print('frontier: ', frontier.length())
    return temp
        ...
        if node[-1] not in exploredSet:
            ...
            for action in legalActions(node[-1][0], node[-1][1]):
                ...
                if isFailed(newPosBox):
                    continue
                new_state = (newPosPlayer, newPosBox)
                frontier.push(node + [new_state], cost(node_action) + heuristic(newPosPlayer, newPosBox))
                actions.push(node_action + [action[-1]], cost(node_action) + heuristic(newPosPlayer, newPosBox))
        print('frontier: ', frontier.length())
    # end =  time.time()
    return temp
